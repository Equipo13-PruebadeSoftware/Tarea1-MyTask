...
def apply_filters(username):
    ...
    if apply_filters == 's':
        ...
        if filter_by_title == 's':
            ...
            log_info(f"El usuario {username} filtro por Titulo: {title}")
        else:
            # Filtrado por estado
    ...
    while True:
        ...
        if choice == '1':
            ...
            if authenticate_user(username, password):
                ...
                while True:
                    # Verificar y actualizar tareas atrasadas (tendra mucho costo colocarlo aca?)
                    ...
                    if task_choice == '1': # Crear tarea
                        ...
                        filters = apply_filters(username)
            ...
            if(username.strip() == ""):
                print(f"No se ingresó nombre de usuario")
                log_warning(f"Registro: No se ingresó nombre de usuario")
                continue
            password = input("Ingrese una contraseña: ")
            if(password.strip() == ""):
                print(f"No se ingresó contraseña")
                log_warning(f"Registro: No se ingresó contraseña para usuario")
                continue
